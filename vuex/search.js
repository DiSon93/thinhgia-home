...
export default {
    ...
    mutations: {
        ...
        getProjectItemType(state, data) {
            state.projects = data.dictionaries;
        },
        getProgressItemType(state, data) {
            state.progress = data.dictionaries;
        }
    },
    actions: {
        getProvince: ({ commit }) => {
        ...
        getProjectItemType: ({ commit }) => {
            return new Promise((resolve, reject) => {
                axiosClient({ url: 'dictionary-types/7', method: "GET" })
                    .then(response => {
                        commit('getProjectItemType', response.data.results);
                        resolve(response.data);
                    })
                    .catch(e => {
                        // commit('showError', e.response.data);
        ...
        getProgressItemType: ({ commit }) => {
            return new Promise((resolve, reject) => {
                axiosClient({ url: 'dictionary-types/8', method: "GET" })
                    .then(response => {
                        commit('getProgressItemType', response.data.results);
                        resolve(response.data);
                    })
                    .catch(e => {
                        // commit('showError', e.response.data);
